...
export async function GET(request: NextRequest) {
  try {
    ...
    // 2. Fetch all scheduled posts that are due
    const now = new Date()
    ...
    console.log(`Cron Publisher: Found ${postsToPublish.length} posts due for publishing.`)
    ...
    return NextResponse.json({
      success: true,
      message: `Completed processing ${postsToPublish.length} scheduled post(s).`,
      results
    })
